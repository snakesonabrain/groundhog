...
class SoilParameterSelector(object):
    """
    The ``SoilParameterSelector`` class allows selection of parameters on a ``LogPlotMatplotlib``
    object. The ``LogPlotMatplotlib`` object is specified during initialisation.
    When used in Jupyter notebooks, the ``%matplotlib qt5`` magic command needs to be
    executed to allow the functionality to work. It will not work with inline plots.

    Soil parameter selection can happen for both constant and linear soil parameter variation.
    """
    
    def __init__(self, logplot):
        """
        Initializes the object by setting the ``LogPlotMatplotlib`` on which the parameter selection is
        performed. The ``SoilProfile`` associated with this logplot is also added as an attribute.
        """
        if type(logplot) == LogPlotMatplotlib:
            self.logplot = logplot
            self.soilprofile = logplot.soilprofile
            warnings.warn("Soil parameter selection only works when the magic command \%matplotlib qt is used")
        else:
            raise ValueError("The soil parameter selector only works with LogPlotMatplotlib objects")
    ...
    def select_additional_layers(self, no_additional_layers, panel_no=1, precision=2):
        ...
        for cm accuracy."""
        ax = self.logplot.axes[panel_no]
        xy = plt.ginput(no_additional_layers)

        x = [p[0] for p in xy]
        y = [round(p[1], precision) for p in xy]
        for _y in y:
            for i in range(self.logplot.axes.__len__() - 1):
                line = self.logplot.axes[i+1].plot(
                    self.logplot.axes[i+1].get_xlim(),
                    (_y, _y), color='grey', ls="--")
    ...
    def select_constant(self, panel_no, parametername, units, nan_tolerance=0.1):
        ...
        x = [p[0] for p in xy]
        y = [p[1] for p in xy]
        #line = self.logplot.axes[panel_no].plot(x,y, 'ro')
    ...
    def select_linear(self, panel_no, parametername, units, nan_tolerance=0.1):
        ...
        x = [p[0] for p in xy]
        y = [p[1] for p in xy]
        #line = self.logplot.axes[panel_no].plot(x,y)
